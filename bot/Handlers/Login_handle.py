...
async def login_name(message: Message, state: FSMContext):
    ...
    msg_delete.append(msg.message_id)


@Login_router.message(Login_state.password)
async def login_password(message: Message, state: FSMContext, bot):
    ...
    if token == "Error":
        ...
        msg_delete.append(msg.message_id)
        for id in msg_delete:
            await bot.delete_message(message.chat.id, message_id=id)
